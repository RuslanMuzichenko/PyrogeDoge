...
def bot_job(phone):
    ...
    scheduler.add_job(bot_start, "date", run_date=f'{datetime.now() + timedelta(seconds=5)}')
    scheduler.start()
    try:
        app.run()
    except Exception as error:
...
try:
    ...
    options.add_argument("--incognito")  # попробывать с сделать папку на одну сесию удалять ее в конце, куки будут собираться там
